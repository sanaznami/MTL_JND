...
def create_model(JND_Recon_Models_Path):
    ...
    ModelLAT.load_weights(JND_Recon_Models_Path)
    ...
    LAT = Model(inputs=input_img, outputs=[jnd_out])
    return LAT

##################################################################################################################
def train_model(LAT, X_Train, train_labels, X_Valid, valid_labels, checkpoint_path, csv_log_path, learning_rate, batch_size, epochs, jnd_value):

    # Compile and train the model
    optimizer = keras.optimizers.Adam(lr=learning_rate)
    LAT.compile(optimizer=optimizer,
...
##################################################################################################################
def test_model(LAT, X_Test, test_labels, result_path, model_weights_path, jnd_value):
    ...
    model_weights_file = os.path.join(model_weights_path, f'LAT{jnd_value}.h5')
...
def main(base_command, jnd_value, data_dir, checkpoint_path, csv_log_path, result_path, learning_rate, batch_size, epochs, model_weights_path, jnd_model_path):
    ...
    if base_command == 'train':
        ...
        LAT = create_model(jnd_model_file)
        
        train_model(LAT, X_Train, train_labels, X_Valid, valid_labels, checkpoint_path, csv_log_path, learning_rate, batch_size, epochs, jnd_value)
        ...
        test_model(LAT, X_Test, test_labels, result_path, model_weights_path, jnd_value)
