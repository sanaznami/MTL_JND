...
def create_model(JND_Recon_Models_Path):
    ...
    ModelREC.load_weights(JND_Recon_Models_Path)
   
    for layer in ModelREC.layers[0:]:
        layer.trainable = False
            
    # Extract REC features
    RECJND = ModelREC.layers[14].output
    
    RECJND = BatchNormalization()(RECJND)
    ...
    REC = Model(inputs=input_img, outputs=[jnd_out])
    return REC

##################################################################################################################
def train_model(REC, X_Train, train_labels, X_Valid, valid_labels, checkpoint_path, csv_log_path, learning_rate, batch_size, epochs, jnd_value):

    # Compile and train the model
    optimizer = keras.optimizers.Adam(lr=learning_rate)
    REC.compile(optimizer=optimizer,
    ...
    history = REC.fit(X_Train,
                      {'JND_output': train_labels[:, jnd_index]},
                      epochs=epochs,
                      batch_size=batch_size,
                      validation_data=(X_Valid, {'JND_output': valid_labels[:, jnd_index]}),
                      callbacks=[checkpoint, csv_logger],
                      shuffle=True)


##################################################################################################################
def test_model(REC, X_Test, test_labels, result_path, model_weights_path, jnd_value):
    ...
    model_weights_file = os.path.join(model_weights_path, f'REC{jnd_value}.h5')
    REC.load_weights(model_weights_file)

    results = REC.predict(X_Test)
    print(results)

    save_test_results(results, result_path, jnd_value)
...
def main(base_command, jnd_value, data_dir, checkpoint_path, csv_log_path, result_path, learning_rate, batch_size, epochs, model_weights_path, jnd_model_path):
    ...
    if base_command == 'train':
        ...
        jnd_index = int(jnd_value[-1]) - 1  # Extract the index of the selected JND value
        jnd_model_file = os.path.join(jnd_model_path, f'{jnd_value}Reconstruction.h5')
        REC = create_model(jnd_model_file)
        
        train_model(REC, X_Train, train_labels, X_Valid, valid_labels, checkpoint_path, csv_log_path, learning_rate, batch_size, epochs, jnd_value)
            
    elif base_command == 'test':
        if not os.path.exists(result_path):
            os.makedirs(result_path)
        
        jnd_index = int(jnd_value[-1]) - 1  # Extract the index (1-based) of the selected JND value
        jnd_model_file = os.path.join(jnd_model_path, f'{jnd_value}Reconstruction.h5')
        REC = create_model(jnd_model_file)
        
        test_model(REC, X_Test, test_labels, result_path, model_weights_path, jnd_value)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--JND_Recon_Models_Path', required=True, type=str, help='Path to the pre-trained JND-quality image Reconstruction models')

    args = parser.parse_args()
    main(args.base_command, args.jnd_value, args.data_dir, args.checkpoint_path, args.csv_log_path, args.result_path, args.learning_rate, args.batch_size, args.epochs, args.model_weights_path, args.JND_Recon_Models_Path)
